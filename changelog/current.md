...
- **QwenPaw HITL persistence**: Persist bounded approval state atomically, restore non-expired requests after runtime restart, retry unavailable Matrix notifications, and flush the state before Worker shutdown. ([191c2af1](https://github.com/wanghaowei06-ui/AgentSwarm/commit/191c2af1))
...
- **Embedded Manager OpenAI-compatible endpoint propagation**: Forward `AGENTTEAMS_OPENAI_BASE_URL` into the Manager container so configured providers such as DeepSeek remain reachable after Manager recreation. ([2e1d76d4](https://github.com/wanghaowei06-ui/AgentSwarm/commit/2e1d76d4))
...
- **TeamHarness Matrix Markdown fallback**: Render horizontal rules and ordered lists correctly when `markdown-it` is unavailable, without inserting invalid block-level `<br>` separators. ([2e1d76d4](https://github.com/wanghaowei06-ui/AgentSwarm/commit/2e1d76d4))
...
- **Matrix Team room cleanup**: Make Manager force-leave commands membership-aware and keep Team Worker room cleanup owned by Team reconciliation, preventing repeated `is not joined in the room` admin errors. ([2e1d76d4](https://github.com/wanghaowei06-ui/AgentSwarm/commit/2e1d76d4))
