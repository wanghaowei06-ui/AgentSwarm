...
- 2026-09-03 `071d3e2` 已将 `P1_BUILD_PROVENANCE` 源码收口（待独立镜像重建）：staging 限定为已跟踪 `qwenpaw/src/qwenpaw_worker/worker.py`，构建前校验固定 Git blob `214b2649…` 与 SHA-256 `f06e91c6…`，并把双哈希 provenance 文件带入镜像后再次校验。旧工作树和 `__pycache__` 不再进入新构建上下文；生产镜像尚未重建，运行态仍 `RUNTIME_NOT_VERIFIED`。
...
- 2026-09-03 `80a9a55` 已完成上述 `P1_ARTIFACT_DIR_TOCTOU` 源码收口：workspace/结果目录保持 FD 打开，目录相对 `O_NOFOLLOW|O_EXCL` 创建，既有文件通过 FD 做属主/类型/哈希校验，并补充 symlink 竞态回归测试。聚焦 adapter 50 项、全量 TestWeaver+脚本 342 项通过；尚未重建/部署生产 Worker 镜像，故运行态仍登记 `SOURCE_HARDENED / RUNTIME_NOT_VERIFIED`，不升级 Hero 分类。
