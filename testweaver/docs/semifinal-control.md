...
- `b28baee` 独立复核曾发现两个 P1：构建 staging 复制整个 `qwenpaw_worker` 工作树且只做 marker smoke，可能携带 ignored `__pycache__`/本机路径；结果目录按路径名创建 artifact，存在 symlink TOCTOU。两项历史风险均保留作审计来源，后续收口如下。
- 2026-09-03 `071d3e2` 已将 `P1_BUILD_PROVENANCE` 源码收口：staging 限定为已跟踪 `qwenpaw/src/qwenpaw_worker/worker.py`，构建前校验固定 Git blob `214b2649…` 与 SHA-256 `f06e91c6…`，并把双哈希 provenance 文件带入镜像后再次校验；旧工作树和 `__pycache__` 不再进入新构建上下文。`6c24fb1` 进一步把只读 provenance 文件保留在最终镜像中，供运行时回读；镜像 `agentteams/qwenpaw-worker:m1plus-6c24fb1-dsh-v4`（digest `sha256:70e893617c0f4c0d5aa8571b80ac548522b8b1bb88a8ad09f84592071e02c7c3`）已本机构建，provenance/DSH/Codex CLI runtime smoke PASS，尚未受控部署，当前 Hero 不升级分类。
- 2026-09-03 `80a9a55` 已完成 `P1_ARTIFACT_DIR_TOCTOU` 源码收口：workspace/结果目录保持 FD 打开，目录相对 `O_NOFOLLOW|O_EXCL` 创建，既有文件通过 FD 做属主/类型/哈希校验，并补充 symlink 竞态回归测试。聚焦 adapter 50 项、全量 TestWeaver+脚本 342 项通过；该项源码已收口，但仍不把未部署镜像写成运行态 PASS。
...
- 2026-09-03 `889039b` 增加 trace-native `CreateEvaluationTask` 接线：真实同一 campaign/run/trace 的任务创建、归属/范围/evidence binding 回读和终态均已被 AgentLoop 接受；backfill 返回 0 条源记录，故仍为 `NOT_VERIFIED`，不把 API 接受误写成 LIVE 评估。v13 离线包（SHA-256 `7876990b24e3a31648051576db8d0c607dc41225c1e8df0ba845ea55bbbad563`）已纳入该安全事实、artifact 竞态修复、QwenPaw 来源 provenance、最终镜像回读与 dashboard 验证记录，`verify`/`replay` 通过且整体仍为 `PARTIAL`。下一系统阻塞是同一真实 Trace 的 AgentLoop/SLS 源记录摄取与可回读，不再扩展采集器。
