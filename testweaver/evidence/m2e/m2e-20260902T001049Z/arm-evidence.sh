...
docker exec "$controller_container" sh -c 'agt get projects -o json' 2>/dev/null \
    | jq '{total, projects: [.projects[]? | {project_id, status, team_id, title, plan_type, mode}]}' \
...
while IFS= read -r project_id; do
    ...
    docker exec "$controller_container" agt get projects "$project_id" -o json 2>/dev/null \
        | jq '{project_id, status, team_id, title, plan_type, mode, nodes: [.nodes[]? | {id, name, status, assignee}]}' \
        > "$state_dir/project-$project_id.json"
done < <(jq -r '.projects[]? | select(.status == "active") | .project_id // empty' "$state_dir/projects.json")
...
manifest = {
    "schema": "testweaver.m2e.evidence-boundary/v1",
    ...
    "project_inventory": {
        "summary": evidence_path(str(state_dir / "projects.json")),
        "active_project_details": sorted(
            evidence_path(str(path))
            for path in state_dir.glob("project-*.json")
        ),
    },
    "teams": team_records,
