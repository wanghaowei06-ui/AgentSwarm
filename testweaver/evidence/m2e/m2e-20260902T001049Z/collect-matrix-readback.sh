...
jq -n \
    ...
    '{schema: "testweaver.m2e.human-event-hash/v1", run_id: $run_id, room_id: $room_id, event_id: $event_id, sender: $sender, origin_server_ts: ($origin_server_ts|tonumber), body_sha256: $body_sha256, event_file_sha256: $event_file_sha256, event_path: $path, values_redacted: true}' \
...
write_stage_observation() {
    ...
    jq -s -c \
        ...
        | {
            schema: "testweaver.m2e.matrix-stage-observation/v1",
            run_id: $run_id,
...
collect_once() {
    ...
    jq -n \
        --arg schema 'testweaver.m2e.matrix-readback/v1' \
        --arg run_id "$run_id" \
