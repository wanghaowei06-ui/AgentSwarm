...
human_input_hash='33bdc27f…e074aa5'
...
worker_specs=(
    'native-m0-clean-leader|agentteams-native-m0-20260901-worker-native-m0-clean-leader|13687'
    'native-m0-clean-worker|agentteams-native-m0-20260901-worker-native-m0-clean-worker|17507'
    'native-m1-verify-leader|agentteams-native-m0-20260901-worker-native-m1-verify-leader|16346'
    'native-m1-verify-worker|agentteams-native-m0-20260901-worker-native-m1-verify-worker|19810'
)

for skill in "${skill_names[@]}"; do
    test -r "$repo_root/testweaver/skills/$skill/SKILL.md"
done
test -r "$redactor"
...
for spec in "${worker_specs[@]}"; do
    ...
    if curl -fsS --max-time 5 "http://127.0.0.1:$port/api/skills" 2>/dev/null \
    ...
    for skill in "${skill_names[@]}"; do
        source_path="$repo_root/testweaver/skills/$skill/SKILL.md"
        source_sha="$(sha256sum "$source_path" | awk '{print $1}')"
        source_commit="$(git -C "$repo_root" log -1 --format=%H -- "testweaver/skills/$skill/SKILL.md")"
...
def relative(path_text):
    try:
        return str(Path(path_text).resolve().relative_to(repo_root.resolve()))
    except ValueError:
        return path_text
...
manifest = {
    "schema": "testweaver.m1plus.evidence-boundary/v1",
