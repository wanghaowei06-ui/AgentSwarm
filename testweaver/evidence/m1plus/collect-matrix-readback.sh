...
room_path='%21hNaeZwz6TWUHhA6I97%3Amatrix-native-m0-20260901.agentteams.local%3A28080'
event_path='%24Q3Ax2m6ZOJmHaNHfN4kmtAujljMXIEzuu7avi7kA4hU'
...
record_event_lines() {
    ...
    while IFS= read -r event; do
        ...
        jq -cn \
            ...
            '{collected_at: $collected_at, source: $source, raw_event_path: "testweaver/evidence/m1plus/matrix-events.jsonl", event_id: $event_id, room_id: $room_id, sender: $sender, origin_server_ts: $origin_server_ts, type: $type, captured_event_sha256: $event_sha256}' \
            >> "$event_index_path"
...
write_stage_observation() {
    ...
    jq -c -s \
        ...
        | ([$post[] | select(((.sender // "") | test("native-m1-verify-leader")))] ) as $leader_events
        | ([$post[] | select(((.sender // "") | test("native-m1-verify-worker")))] ) as $worker_events
        | ([$post[] | select(
              ((.sender // "") | test("native-m1-verify-leader"))
              and (body | test("project"; "i"))
              and (body | test("task"; "i"))
          )]) as $delegation_events
        | ([$post[] | select(
              (((.sender // "") == $manager_sender) or ((.sender // "") | test($manager_pattern)) or ((.sender // "") | test("native-m1-verify-leader")))
